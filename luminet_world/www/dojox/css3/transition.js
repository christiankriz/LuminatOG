define(["dojo/_base/lang",
	...
	transition.zoom = function(node, config){
		// summary:
		//		Method which is used to create the transition object of zoom effect.
		// node:
		//		The node that the zoom transition effect will be applied on.
		// config:
		//		The cofig arguments which will be mixed into this transition object.
		var ret = new transition(config);
		ret.node = node;
		...
		if(ret["in"]){
			if(ret.direction == 1){
				...
				ret.startState[transitionPrefix + "ransform"]="scale(0.01,0.01)";
				ret.endState[transitionPrefix + "ransform"]="scale(1,1)";
			}else{
				lang.mixin(ret, {
					startState:{
					"opacity": "0"
				...
				ret.startState[transitionPrefix + "ransform"]="scale(2,2)";
				ret.endState[transitionPrefix + "ransform"]="scale(1,1)";
			}
		}else{
			...
			if(ret.direction == 1){
				...
				ret.startState[transitionPrefix + "ransform"]="scale(1,1)";
				ret.endState[transitionPrefix + "ransform"]="scale(2,2)";
			}else{
				lang.mixin(ret, {
					startState:{
					"opacity": "1"
				...
				ret.startState[transitionPrefix + "ransform"]="scale(1,1)";
				ret.endState[transitionPrefix + "ransform"]="scale(0.01,0.01)scale(1,1)";
			}
		}
	...
	transition.rotate = function(node, config){
		// summary:
		//		Method which is used to create the transition object of zoom effect.
		// node:
		//		The node that the zoom transition effect will be applied on.
		...
		if(ret["in"]){
			if(ret.direction == -1){
				...
				ret.startState[transitionPrefix + "ransform"]="rotate(90deg)";
				ret.endState[transitionPrefix + "ransform"]="rotate(0deg)";
			}else{
				lang.mixin(ret, {
					startState:{
					"opacity": "0"
